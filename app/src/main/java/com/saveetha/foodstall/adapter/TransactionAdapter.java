...
public class TransactionAdapter extends RecyclerView.Adapter<TransactionAdapter.TransactionViewHolder> {
    ...
    public void onBindViewHolder(@NonNull TransactionViewHolder holder, int position) {
        ...
        if (transaction.amount.startsWith("-")) {
            holder.amount.setTextColor(holder.itemView.getResources().getColor(android.R.color.holo_red_dark));
        } else {
            holder.amount.setTextColor(holder.itemView.getResources().getColor(android.R.color.holo_green_dark));
        }
        // You would typically handle status text color based on the status field
    }
